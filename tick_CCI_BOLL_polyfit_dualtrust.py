...
def run(st,ed,onruncontract=None):
    ...
    iif( ((前1<=move(middle,1) and 最新价>middle) or (最新价<lower)),'buy',iif( ((前1>=move(middle,1) and 最新价<middle) or (最新价>upper)) ,'sell','TODO' )   )   as `BOLL_flag ,
...
'''
    ...
    qout='''
       fulll=table(10000000:5, `当前时间`合约名称`最新价`方程值`CCI方向`BOLL方向, `TIMESTAMP`SYMBOL`DOUBLE`DOUBLE`SYMBOL`SYMBOL)//;(当前时间, 合约名称,最新价,方程值,方向 )
  for ( i in tq) {
 try{
...
if(n[3]>l[3]){ //print "下穿则卖出 " 
 	...
 	aw=yhat-i.CCI
		 		 		 if ( aw==NULL ){continue };
		 		 		 if (i.最新价<i.Sellline){
 fulll.append!( table([[ i.当前时间 ],[i.合约名称],[i.最新价],[aw],["下穿则卖出"]  ,[i.BOLL_flag ] ])   )
 }else{continue}
}
...
t=( i.int_当前时间/1000000000-int( l[0][:10] ))/( i.int_当前时间/1000000000- int( n[0][:10]))
 	...
 	aw=yhat-i.CCI
		// aw=分母-分子;
		 		 		 if ( aw==NULL ){continue };
if  (  i.最新价>i.Buyline ){
 fulll.append!( table([ [i.当前时间],[i.合约名称],[i.最新价],[aw],["上穿则买入"] ,[i.BOLL_flag ]  ]  ) )
 }else{ continue }
 	} }catch(ex){}	
 //	}else{};
 }
...
select top 1 当前时间,合约名称,最新价,iif( ((CCI操作=="buy") and (BOLL方向 =="buy")),"buy",iif( ((CCI操作=="sell") and (BOLL方向 =="sell")),"sell","TODO"  )) as `操作 from fulll context  by 合约名称 csort 当前时间 desc having 当前时间>=(now()+1000*3600*8-1000*60*2.5)
//select top 100000 当前时间,合约名称,最新价,iif( ((CCI操作=="buy") and (BOLL方向 =="buy")),"buy",iif( ((CCI操作=="sell") and (BOLL方向 =="sell")),"sell","TODO"  )) as `操作 from fulll context  by 合约名称 csort 当前时间 desc
...
#     main()
    ...
    con.run(获取数据.format(bBandsperid='14',
                            ...
                            ins=['豆油2205','豆一2205',"豆粕2205",'玉米2205','鸡蛋2205','梗米2205'][:]))
    con.run('''
...
update all3 set 日期= all3[`时间]
    ...
    fetchins.update({"data":['y2205','a2205','jd2205','c2205','m2205','rr2205']})
    # 初始金额=10000/len(ins)
    # main()
    ##订阅CTP行情
