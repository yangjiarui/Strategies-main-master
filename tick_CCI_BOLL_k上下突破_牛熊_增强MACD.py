...
//增加牛熊指标
   ...
   all=select top 1000000  *,最新价<REF(最新价,1) and 最新价<REF(最新价,2) as `VARD
                   ,最新价>REF(最新价,1) and 最新价>REF(最新价,2) as `VAR1,MACD( 最新价,5,13,9)[0] as `short_dif , MACD( 最新价,5,13,9)[1] as `short_dea , MACD( 最新价,5,13,9)[2] as `short_macd
  , MACD( 最新价,24,47,9)[0] as `mid_dif ,   MACD( 最新价,24,47,9)[1] as `mid_dea  , MACD( 最新价,24,47,9)[2] as `mid_macd
                   from all2 context by 合约名称 csort 当前时间 

        all=select top 1000000  *,REF(VARD,1) and 最新价>=REF(最新价,1) and 最新价<=REF(最新价,2) as `VARE
                   ,REF(VAR1,1) and 最新价<=REF(最新价,1) and 最新价>=REF(最新价,2) as `VAR2
  from all  context by 合约名称 csort 当前时间 


        all=select top 1000000  *
                   ,REF(VAR2,1) and 最新价>=REF(最新价,1) and 最新价<=REF(最新价,2) as `VAR3 , REF(VARE,1) and 最新价<=REF(最新价,1) and 最新价>=REF(最新价,2) as `VARF
  from all context by 合约名称 csort 当前时间 

        all=select top 1000000  *
                   ,REF(VAR3,1) and 最新价<=REF(最新价,1) and 最新价>=REF(最新价,2) as `VAR4 , REF(VARF,1) and 最新价>=REF(最新价,1) and 最新价<=REF(最新价,2) as `VAR10
  from all context by 合约名称 csort 当前时间 

        all=select top 1000000  *
                   , REF(VAR4,1) and 最新价>=REF(最新价,1) and 最新价<=REF(最新价,2) as `VAR5 , REF(VAR10,1) and 最新价<=REF(最新价,1) and 最新价>=REF(最新价,2) as `VAR11
  from all  context by 合约名称 csort 当前时间 

        all=select top 1000000  *
                   , REF(VAR5,1) and 最新价<=REF(最新价,1) and 最新价>=REF(最新价,2) as `VAR6  , REF(VAR11,1) and 最新价>=REF(最新价,1) and 最新价<=REF(最新价,2) as `VAR12
  from all  context by 合约名称 csort 当前时间 

        all=select top 1000000  *
                   , REF(VAR6,1) and 最新价>=REF(最新价,1) and 最新价<=REF(最新价,2) as `VAR7  , REF(VAR12,1) and 最新价<=REF(最新价,1) and 最新价>=REF(最新价,2) as `VAR13
  from all  context by 合约名称 csort 当前时间 

        all=select top 1000000  *
                   ,REF(VAR7,1) and 最新价<=REF(最新价,1) and 最新价>=REF(最新价,2) as `VAR8   , REF(VAR13,1) and 最新价>=REF(最新价,1) and 最新价<=REF(最新价,2) as `VAR14
  from all context by 合约名称 csort 当前时间 


        all=select top 1000000  *
                   ,REF(VAR8,1) and 最新价>=REF(最新价,1) and 最新价<=REF(最新价,2)  as `VAR9  , REF(VAR14,1) and 最新价<=REF(最新价,1) and 最新价>=REF(最新价,2) as `VAR15
  from all  context by 合约名称 csort 当前时间 


        all=select top 1000000  *
                   , REF(VAR9,1) and 最新价<=REF(最新价,1) and 最新价>=REF(最新价,2) as `VARA  , REF(VAR15,1) and 最新价>=REF(最新价,1) and 最新价<=REF(最新价,2) as `VAR16
  from all  context by 合约名称 csort 当前时间 


        all=select top 1000000  *
                   ,  REF(VARA,1) and 最新价>=REF(最新价,1) and 最新价<=REF(最新价,2) as `VARB    , REF(VAR16,1) and 最新价<=REF(最新价,1) and 最新价>=REF(最新价,2) as `VAR17
  from all context by 合约名称 csort 当前时间 

        all=select top 1000000  *
                    ,  REF(VAR17,1) and 最新价>=REF(最新价,1) and 最新价<=REF(最新价,2) as `VAR18
  from all  context by 合约名称 csort 当前时间 

        all=select top 1000000  *
                   , REF(VARB,1) and 最新价<=REF(最新价,1) and 最新价>=REF(最新价,2) as `VARC   , REF(VARD or VARE or VARF or VAR10 or VAR11 or VAR12 or VAR13 or VAR14 or VAR15 or VAR16 or VAR17 or VAR18,1) and VAR1 as `VAR19
  from all  context by 合约名称 csort 当前时间 
        all=select top 1000000  *
 ...
 all2=select top 1000000  *
                 , iif( VAR19==1,`buy,iif( VAR1A==1,`sell,`TODO     )         ) as `操作1
                 , iif(short_dea>=0 and short_dif>=0 and CROSS(short_dea,short_dif )==1,`sell,iif( mid_dea<=0 and mid_dif<=0  and CROSS( mid_dif,mid_dea  ) ==1 ,`buy     ,`TODO  ) ) as `操作2
  from all  context by 合约名称 csort 当前时间 
...
//   tq=select top 100000 *  ,NONE as `CCIflag, NONE as `方程 ,moving(dynimic,[int_当前时间,CCI],flag ) as `calulast,moving(dynimic,[int_当前时间,CCI],flag-4 ) as `calunow 
   ...
   ,iif((上影线长度>实体线长度>3 and 2<1.65*下影线长度<上影线长度),"sell",iif((下影线长度>实体线长度>3 and 下影线长度>上影线长度*1.65>2),"buy","TODO" )) as `影线操作
   ,iif( 
   ...
   (flag线=="阳线" and move(flag线,1)=="阴线" and move(flag线,2)=="阳线" and move(flag线,3)=="阴线" and 最新价>move(开盘价,1) and move(开盘价,1)>move(最新价,2) and 实体线长度>move(实体线长度,1)>move(实体线长度,2)>0  and move(最高价,1) >= move(最高价,2) ),"buy"   
      ,"TODO" )) as `三兵指标
      ,  iif(momflag==`buy and mombetaflag ==`mom_take ,`buy,  iif( momflag==`sell and mombetaflag ==`mom_take,`sell,`TODO               )           )   as `mom操作 
...
'''
    ...
    qout='''
//select top 1 当前时间,合约名称,最新价,
//iif( (((CCI操作=="buy") and (影线操作 =="buy") and (BOLL_flag="buy")) or 三兵指标==`buy or mom操作==`buy) and 操作1==操作2==`buy,"buy",iif( (((CCI操作=="sell") and (影线操作 =="sell") and (BOLL_flag="sell"))  or 三兵指标==`sell or mom操作==`sell )  and 操作1==操作2==`sell,"sell","TODO"  )) as `操作 from tq context  by 合约名称 csort 当前时间  desc  having (( 当前时间.hour()>=9 and 当前时间.minute()>=43 and 当前时间.hour()<=15) or ( 当前时间.hour()>=21 and 当前时间.minute()>=15 and 当前时间.hour()<=23)) and 当前时间>=(now()-1000*60*0.95);

select top 1 当前时间,合约名称,最新价,
iif( (((CCI操作=="buy") and (影线操作 =="buy") and (BOLL_flag="buy")) or 三兵指标==`buy or mom操作==`buy) and 操作1==操作2==`buy,"buy",iif( (((CCI操作=="sell") and (影线操作 =="sell") and (BOLL_flag="sell"))  or 三兵指标==`sell or mom操作==`sell )  and 操作1==操作2==`sell,"sell","TODO"  )) as `操作 from tq context  by 合约名称 csort 当前时间  desc  having (( 当前时间.hour()>=9 and 当前时间.minute()>=43 and 当前时间.hour()<=15) or ( 当前时间.hour()>=21 and 当前时间.minute()>=15 and 当前时间.hour()<=23)) and 当前时间>=(now()-1000*60*0.95);

//select top 100000 当前时间,合约名称,最新价, iif(momflag==`buy and mombetaflag ==`mom_take and (BOLL_flag="buy") ,`sell,  iif( momflag==`sell and mombetaflag ==`mom_take and (BOLL_flag="sell"),`sell,iif( ((CCI操作=="buy") and (影线操作 =="buy") and (BOLL_flag="buy")),`buy,`TODO            )               )           )   as `操作  from tq context  by 合约名称 csort 当前时间 desc having ( 当前时间.hour()>=9 and 当前时间.minute()>=43 and 当前时间.hour()<=15) or ( 当前时间.hour()>=21 and 当前时间.minute()>=15 and 当前时间.hour()<=23) and 当前时间>=(now()-1000*60*0.95);
'''
    if showsql:
        print(qout)
    try:
...
def main():
    ...
    try:
        ...
        st,ed=pre_con(st=(now-datetime.timedelta(days=0,hours=1,minutes=35,seconds=20)),ed=(now+datetime.timedelta(hours=0,minutes=0)))
        ###macd_inverse
        ...
        if ( (len(last) if last else []) != len(flagres) ) or any(last!=flagres ) :
            ...
            for i in filter(lambda x:x.get('合约名称') in c,res):
                ...
                if i.get('操作') == 'TODO':
                    ...
                    buy(contractname=i.get('合约名称'),currentprice=lastprice.get(i.get('合约名称'))+买滑点,flag=4000)
                    email=True
                elif i.get('操作') == 'sell':
                    xgb_res=filter(lambda x:x.get('code')==i.get('合约名称'),xgb).__next__()
                    ...
                    sell(contractname=i.get('合约名称'),currentprice=lastprice.get(i.get('合约名称'))-卖滑点,flag=4000)
                    email=True
...
def tick_CCI_BOLL_k上下突破():
    ...
    con.run(获取数据.format(bBandsperid='14',
                        ...
                        ins=["豆粕2301",'玉米2301','鸡蛋2301','聚乙烯2301','RM2301','纸浆2301','豆油2301','螺纹钢2301'][:]))
    con.run('''
...
update all3 set 日期= all3[`时间]
    ...
    fetchins.update({"data":['l2301','jd2301','c2301','m2301','RM301','sp2301','y2301','rb2301']})
    # 初始金额=10000/len(ins)
    # main()
    ...
    time.sleep(1)
    sched1.add_job(main,'interval',seconds=25)
    try:
        sched1.start()
    except Exception as e:
        print('sched1.exit()')
