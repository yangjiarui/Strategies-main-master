...
,iif(最新价>move(最新价,n),1,iif(最新价<move(最新价,n),-1,0)) as `td
from all context by 种名简称,合约名称 csort 当前时间;
all=select top 1000000 *,cumsum(td) ,take(00l, all.count()) as `td_cum_start,move(td,1) as `前td , move(cumsum(td),1) as `前cumtd  from all context by  种名简称,合约名称 csort 当前时间;

update all set td_cum_start=前cumtd where td!=前td ;


all= select top 1000000 * ,  (cumsum_td - td_cum_start.ffill()) as `td_cum_start_flag ,
  iif( (   最新价>=move(最高价,2) and 最高价>move(最高价,1) and 最新价>前1 ) , `td_buy, iif( (  最新价<=move(最低价,2) and 最低价<move(最低价,1) and 最新价<前1 )  ,`td_sell,`TODO  )    ) as `tdcaozuo
from  all context by  种名简称,合约名称 csort 当前时间;

all3=select 合约名称,当前时间,最新价,tdcaozuo,td_cum_start_flag,0 as `buyCount,0 as `sellCount from   all
	buy_count=0;
	buy_flag=0;
	sell_count=0;
	sell_flag=0;
for ( i in 0..(all3.count()-1)){{
//	print( i  )
	if ( all3[i,4]==-n2  ){{  //买计数
		buy_flag=1;
		buy_count=0;
...
if (buy_flag  and all3[i,3]==`td_buy){{     buy_count+=1 ; update all3  set buyCount=buy_count where 当前时间=all3[i,1] and 合约名称=all3[i,0]       }};
if (sell_flag and all3[i,3]==`td_sell ){{     sell_count+=1  ;  update all3  set sellCount=sell_count where 当前时间=all3[i,1] and 合约名称=all3[i,0]     }};		
	}}
all=lj(all,all3,`当前时间`合约名称)
...
'''
    ...
    qout='''
       full=table(10000000:5, `当前时间`合约名称`最新价`方程值`CCI方向`BOLL方向`buyCount`sellCount, `TIMESTAMP`SYMBOL`DOUBLE`DOUBLE`SYMBOL`SYMBOL`DOUBLE`DOUBLE)//;(当前时间, 合约名称,最新价,方程值,方向 )
  for ( i in tq) {
...
//		 aw=分母-分子;
 ...
 full.append!( table([[ i.当前时间 ],[i.合约名称],[i.最新价],[aw],["下穿则卖出"]  ,[i.BOLL_flag ] , [i.buyCount], [i.sellCount]])   )
 }else{continue}
}
...
if  ( (i.拟合flag !=0 and i.dydt*i.前dydt<0) or (i.拟合flag !=0 and i.dy2dt2*i.前dy2dt2<0)  ){
 full.append!( table([ [i.当前时间],[i.合约名称],[i.最新价],[aw],["上穿则买入"] ,[i.BOLL_flag ] , [i.buyCount], [i.sellCount]  ]  ) )
 }else{ continue }
 	} }catch(ex){}	
 //	}else{};
 }
 ...
 n3=10
full=select *, iif( ( ( 方程值-move(方程值,1)>0) and ( CCI方向=="上穿则买入" ) ) ,"buy" , iif( ( and ( 方程值-move(方程值,1)<0) and ( CCI方向=="下穿则卖出" ) ),"sell" ,"TODO") ) as `CCI操作 from full context  by 合约名称 csort 当前时间 ;
...
//full = select * from full where  ( 当前时间.hour()>=9 and 当前时间.minute()>=50 and 当前时间.hour()<=15) or ( 当前时间.hour()>=21 and 当前时间.minute()>=45 and 当前时间.hour()<=23);
//select top 1 当前时间,合约名称,最新价,iif( ((CCI操作=="buy") and (BOLL方向 =="buy")),"buy",iif( ((CCI操作=="sell") and (BOLL方向 =="sell")),"sell","TODO"  )) as `操作 from full context  by 合约名称 csort 当前时间 desc having 当前时间>=(now()+1000*3600*8-1000*60*1.5)
//select top 1 当前时间,合约名称,buyCount,sellCount,BOLL_flag,最新价,iif( buyCount>=n3 and BOLL_flag =="buy","buy",iif( BOLL_flag =="sell" and sellCount>=n3,"sell","TODO"  )) as `操作 from tq context  by 合约名称 csort 当前时间 desc // having 当前时间>=(now()+1000*3600*8-1000*60*2.8)
//select top 1 当前时间,合约名称,buyCount,sellCount,BOLL_flag,最新价,iif( buyCount>=n3 and BOLL_flag =="buy","buy",iif(  sellCount>=n3 and BOLL_flag =="sell","sell","TODO"  )) as `操作 from tq context  by 合约名称 csort 当前时间 desc having 当前时间>=(now()+1000*3600*8-1000*60*1.8)

select top 100000 当前时间,合约名称,buyCount,sellCount,BOLL_flag,最新价,iif( buyCount>=n3 ,"buy",iif(  sellCount>=n3,"sell","TODO"  )) as `操作 from tq context  by 合约名称 csort 当前时间 desc  //having 当前时间>=(now()+1000*3600*8-1000*60*1.8)

'''
    if showsql:
        print(qout2)
    res=con.run(qout2)
    # con.run('''select top 10000 当前时间,合约名称,最新价,iif( buyCount>=n3 and BOLL_flag =="buy","buy",iif( BOLL_flag =="sell" and sellCount>=n3,"sell","TODO"  )) as `操作 from tq context  by 合约名称 csort 当前时间 desc ''').to_csv('res.csv',encoding='utf-8-sig')
    ...
    # print(res)
    ###集成了xgboost算法
    xgb=load_xgboost(con=con,allcode=res['合约名称'].to_list(),st=(ed-datetime.timedelta(days=0,hours=1,seconds=20*60)),ed=ed)
    # res.to_csv("res.csv",encoding='utf-8-sig')
...
#
    ...
    st,ed=pre_con(st=(now-datetime.timedelta(days=10,hours=5,minutes=35,seconds=135)),ed=(now+datetime.timedelta(hours=0,minutes=0)))
    ###macd_inverse
    ...
    if ( (len(last) if last else []) != len(flagres) ) or any(last!=flagres ) :
        ...
        for i in filter(lambda x:x.get('合约名称') in c,res):
            ...
            if i.get('操作') == 'TODO':
                ...
                # buy(contractname=i.get('合约名称'),currentprice=lastprice.get(i.get('合约名称'))+买滑点,flag=10)
                email=True
            elif i.get('操作') == 'sell':
                xgb_res=filter(lambda x:x.get('code')==i.get('合约名称'),xgb).__next__()
                ...
                # sell(contractname=i.get('合约名称'),currentprice=lastprice.get(i.get('合约名称'))-卖滑点,flag=10)
                email=True
...
update all3 set 日期= all3[`时间]
    ...
    fetchins.update({"data":['a2205','jd2205','c2205','m2205','rr2205']})
    # 初始金额=10000/len(ins)
    # main()
    ##订阅CTP行情
...
#     ###实时寻找平仓机会
    ...
    time.sleep(1)
    sched1.add_job(main,'interval',seconds=45)
    try:
        sched1.start()
