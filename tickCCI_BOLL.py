...
//all2=lj(all2,all3,`日期`合约名称)
all2=lj(all2,all3,`合约名称)
all2=select * from all2 where 最新价>all3_lower and 最新价<all3_upper;
   tq=select top 10000 *  ,NONE as `CCIflag, NONE as `方程 ,moving(dynimic,[int_当前时间,CCI],flag ) as `calulast,moving(dynimic,[int_当前时间,CCI],flag-4 ) as `calunow  from all2 context by 合约名称 csort 当前时间 asc
   nullFill!(tq,0)
'''
    if showsql:
        print(q)
...
//		 aw=分母-分子;
		 t=( i.int_当前时间-timestamp( l[2] ))/( i.int_当前时间- timestamp( n[2]))
...
t=( i.int_当前时间-timestamp( l[0] ))/( i.int_当前时间- timestamp( n[0]))
...
select top 1 当前时间,合约名称,最新价,iif( ((CCI操作=="buy") and (BOLL方向 =="buy")),"buy",iif( ((CCI操作=="sell") and (BOLL方向 =="sell")),"sell","TODO"  )) as `操作 from full context  by 合约名称 csort 当前时间 desc having 当前时间>=(now()+1000*3600*8-1000*8)
'''
    if showsql:
        print(qout)
    res=con.run(qout)
...
#
    ...
    st,ed=pre_con(st=(now-datetime.timedelta(days=0,hours=0,minutes=10,seconds=135)),ed=(now+datetime.timedelta(hours=0,minutes=0)))
    ###macd_inverse
...
#          """))})
    ...
    sched1.add_job(finding_2_cansole_trade,'interval',seconds=10)
