...
all=select  *, move(成交量,1),move(持仓量,1),move(最新价,-4) as `后4 from futures where 当前时间>={st} and 当前时间 <={ed}  and 种名简称 = 品种名 and  (  insrtument.count()==0 || 合约名称 in insrtument ) order by 当前时间 asc
...
def run(st,ed):
    ...
    q='''
    flag=4
           tq=select top 100000000 *,开盘价-move(开盘价,flag),
最高价-move(最高价,flag),
最低价-move(最低价,flag),
昨日收盘价-move(昨日收盘价,flag),
买价-move(买价,flag),
卖价-move(卖价,flag),
最新价-move(最新价,flag),
买量-move(买量,flag),
...
from all2 context by 合约名称 csort 当前时间 asc
   nullFill!(tq,0)
     ...
     aim=select top 1 * from tq  where {} context by 合约名称 csort 当前时间 desc  
            '''

    conditions=['持仓量','成交量']
    thres=0.25
    lis=' and '.join( (f"(  {i}_sub > moving(mean,{i}_sub,flag)-{thres}*moving(std,{i}_sub,flag) and {i}_sub < moving(mean,{i}_sub,flag)+{thres}*moving(std,{i}_sub,flag) )" for i in conditions) )
    if showsql:
        print(q.format(lis))
    con.run(q.format(lis))
