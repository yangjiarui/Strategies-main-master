...
allt=select 最新价 from all2  pivot by 当前时间,合约名称
allt.nullFill!(0)
...
allt=select *, abs( {contract1}-{contract2} ) as `absdiff from allt
pair=select avg({contract1}) ,avg({contract2})  from allt
try {{ allt=select *,abs(absdiff-currentdiff) as `sub from allt
flag=0
}}catch(ex){{
	currentdiff=allt[`absdiff].mean();
	allt=select *,abs(absdiff-currentdiff) as `sub from allt 
	flag=1 
	}};
	// allt=select top {look_pre}:10000000 *,iif( (absdiff<moving(mean,absdiff,{look_pre})-0.25*moving(std,absdiff,{look_pre})) ,"差值变大",iif( (absdiff>moving(mean,absdiff,{look_pre})+0.25*moving(std,absdiff,{look_pre}) ),"差值变小","TODO" ) ) as `sing   from allt where sub>{sub} order by 当前时间 asc
// nowdiff=allt[`sing].last()
n=allt.当前时间.last()+':00'
...
if ( (pair[`avg_{contract1}]>pair[`avg_{contract2}]) and (flag==1)   ){{  
	full.append!(  table([[timestamp(n)],[`{contract1}],[0],["sell"]   ]) ) ;
             ...
             }}else if( (flag==0) and (  sub>{sub} ) and  (sellclose != buyclose )   and ( currentdiff>allt.absdiff.last()   )    ){{   // and ( currentdiff>allt.absdiff.last()   )  
         //    	try{{
  full.append!(  table([[ timestamp(n)  ],[sellclose],[0],["sell"]   ]) );
	  full.append!(  table([[ timestamp(n)  ],[buyclose],[0],["buy"]   ]) ); }};
//}}catch(ex){{ }}
 select * from full where 当前时间>= (now()+1000*3600*8-30*60*1000)
'''.format(contract1=cols[1],contract2=cols[2],look_pre=3,sub=0)
    if showsql:
        print(q)
    res=con.run(q)
    print('allt',con.run('allt').iloc[-1,:])
    print('res',res)
...
# 2021.10.22 06:12:04 and 当前时间 <=2021.10.22 16:14:19
    ...
    if ( (len(last) if last else []) != len(flagres) ) or any(last!=flagres ) :
        ...
        for i in filter(lambda x:x.get('合约名称') in c,res):
            ...
            if i.get('操作') == 'TODO':
                ...
                if handstate[i.get('合约名称')]['sell_position']:
                    pbuy(contractname=i.get('合约名称'),currentprice=当前实际价格-卖滑点,flag='close')
                    continue
                ...
                if handstate[i.get('合约名称')]['buy_position']:
                    psell(contractname=i.get('合约名称'),currentprice=当前实际价格-卖滑点,flag='close')
                    continue
        ...
        try:
            qwe=con.run('currentdiff')
        except:
            qwe=0
        spread.update({"time":now,"当前价差":qwe})
        res.append(spread)
...
#     con.connect('server.natappfree.cc',38815,'admin','123456')
    ...
    if len(反向初始化)<=0:
        反向初始化.update({'sellclose':1,"buyclose":1})
    反向初始化.update({"ins":futures_base.ins,
                  })
    con.upload(反向初始化)
    if futures_base.currentdiff:
        con.upload({"currentdiff":futures_base.currentdiff})
    ###订阅CTP行情
