...
allt=select  *,minute(当前时间) as `到分钟 ,timestamp(当前时间) as `int_当前时间,最新价 from futures where 当前时间>={st} and 当前时间 <={ed}  and 种名简称 = 品种名 and  (  insrtument.count()==0 || 合约名称 in insrtument ) order by 当前时间 asc


all=select last(合约名称) as `合约名称 ,first(开盘价) as `开盘价 ,max(最高价) as`最高价  ,min(最低价) as `最低价  , mean(最新价) as `最新价 , mean(买价) as `买价, 
mean(卖价) as `卖价 ,mean(买量) as `买量 ,mean(卖量) as `卖量 ,  mean(持仓量) as `持仓量 , mean(成交量) as `成交量 ,last(商品交易所简称) as `商品交易所简称,last(日期) as `日期,
last(到分钟) as `到分钟  , last(当前时间) as `当前时间,last(种名简称) as `种名简称 from allt group by 日期,合约名称,到分钟
mergeflag={mergeflag}
all=select  top 10000000 last(合约名称) as `合约名称 ,first(开盘价) as `开盘价 ,moving(max,最高价,mergeflag) as`最高价  ,moving(min,最低价,mergeflag) as `最低价  , moving(last,最新价,mergeflag) as `最新价 , moving(last,买价,mergeflag) as `买价, 
moving(last,卖价,mergeflag) as `卖价 ,moving(last,买量,mergeflag) as `买量 ,moving(last,卖量,mergeflag) as `卖量 ,  moving(last,持仓量,mergeflag) as `持仓量 ,moving( last,成交量,mergeflag) as `成交量 , moving(last,商品交易所简称,mergeflag),moving(last,日期,mergeflag) as `日期,moving(last,商品交易所简称,mergeflag) as `商品交易所简称,moving(last,种名简称,mergeflag) as `种名简称,moving(last,timestamp(当前时间),mergeflag)as `int_当前时间,
moving(last,到分钟,mergeflag) as `到分钟  , moving(last,当前时间,mergeflag) as `当前时间 from all context by 合约名称 csort 当前时间;
...
all2,allt=pocess_ins_vol(futures,ins[0])      //select  * from futures where 日期<=2021.12.21  and 当前时间>={st} and 当前时间 <={ed} and 种名简称 = ins[0]
def mergetables( futures,mutable f1,mutable f2 ,lis ){
	for ( i in lis){	
	try{
i1,i2=pocess_ins_vol(futures,i)               //select  * from futures where 日期<=2021.12.21  and 当前时间>={st} and 当前时间 <={ed} and 种名简称 = i
		f1.append!(i1)
		f2.append!(i2)}catch(ex){}
		}
	}
mer=mergetables{ futures,all2,allt}
ploop(mer, ins[1:]  );
'''
    if showsql:
        print(q)
...
//   tq=select top 100000 *  ,NONE as `CCIflag, NONE as `方程 ,moving(dynimic,[int_当前时间,CCI],flag ) as `calulast,moving(dynimic,[int_当前时间,CCI],flag-4 ) as `calunow 
   ...
   ,iif((上影线长度>实体线长度>3 and 1.65*下影线长度<上影线长度),"sell",iif((下影线长度>实体线长度>3 and 下影线长度>上影线长度*1.65),"buy","TODO" )) as `影线操作
   ,iif( 
...
'''
    ...
    qout='''
rrrr=select top 1 当前时间,合约名称,iif( (((CCI操作=="buy") and (影线操作 =="buy") and (BOLL_flag="buy")) or 三兵指标==`buy or mom操作==`buy) and adxflag=`adx_take ,"buy",iif( (((CCI操作=="sell") and (影线操作 =="sell") and (BOLL_flag="sell"))  or 三兵指标==`sell or mom操作==`sell ) and adxflag=`adx_take,"sell","TODO"  )) as `操作 from tq context  by 合约名称 csort 当前时间 desc // having (( 当前时间.hour()>=9 and 当前时间.minute()>=43 and 当前时间.hour()<=15) or ( 当前时间.hour()>=21 and 当前时间.minute()>=15 and 当前时间.hour()<=23)) and 当前时间>=(now()-1000*60*0.95);
...
rrrr=select 当前时间,合约名称,最新价,操作 from lj(rrrr,allt,`当前时间`合约名称) 
select 当前时间,合约名称,最新价,操作 from rrrr where (( 当前时间.hour()>=9 and 当前时间.minute()>=43 and 当前时间.hour()<=15) or ( 当前时间.hour()>=21 and 当前时间.minute()>=15 and 当前时间.hour()<=23)) and 当前时间>=(now()-1000*60*0.95)
...
def tick_CCI_BOLL_k上下突破():
    ...
    con.run(获取数据.format(bBandsperid='14',
                        ...
                        ins=["豆粕2209",'玉米2209','鸡蛋2209','聚乙烯2209','RM2209','纸浆2209','豆油2209','螺纹钢2209'][:]))
    con.run('''
