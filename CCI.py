...
all=select  *, move(成交量,1),move(持仓量,1),move(最新价,-4) as `后4  ,timestamp(当前时间) as `int_当前时间,最新价,ta::cci(最高价,最低价,最新价,14) as `CCI from futures where 当前时间>={st} and 当前时间 <={ed}  and 种名简称 = 品种名 and  (  insrtument.count()==0 || 合约名称 in insrtument ) order by 当前时间 asc
	update all set 成交量=( 成交量 - move_成交量 ) , 持仓量=(  持仓量-move_持仓量) 
return select 合约名称,当前时间,开盘价,最高价,最低价,昨日收盘价,买价,卖价,最新价,结算价,昨结算,买量,卖量,持仓量,成交量,商品交易所简称,种名简称,日期,后4,int_当前时间,CCI from all
}}
...
def run(st,ed):
    global first
    q='''
       tq=select top 1000000 *  ,NONE as `flag, NONE as `fx ,moving(dynimic,[int_当前时间,CCI],flag ) as `calu , move( moving(dynimic,[int_当前时间,CCI],flag ),1  ) as `diffcalu from all2 context by 合约名称 csort 当前时间 asc
   nullFill!(tq,0)
//xmax+'_'+ymax+'_'+xmin+'_'+ymin
...
//update tq set flag=2 where 当前时间=i.当前时间 and 合约名称=i.合约名称} 	
 full.append!( table([ [i.当前时间],[i.合约名称],[i.最新价],[aw],["上穿则买入"]   ]  ) )
    ...
    q='''
    full=select top 1000000 *, move(方程值,1 )  from full  context by 合约名称 csort 当前时间 asc
    select  top 1 当前时间,合约名称,最新价 , iif(  方向==`下穿则卖出, `sell,iif(方向==`上穿则买入,`buy,`没有意义 )) as `操作 from full where (方程值>0 and move_方程值<0 and 方向=`上穿则买入) or (方程值<0 and move_方程值>0 and 方向=`下穿则卖出) and 当前时间>= (now()+1000*3600*8-1000*10) context by 合约名称 csort 当前时间 desc
       // select  top 1 当前时间, 合约名称,最新价 , iif(  最新价>后4, `sell,iif(最新价<后4,`buy,`没有意义 )) as `操作 from aim where  当前时间>= (now()+1000*3600*8-1000*90)  context by 合约名称 csort 当前时间 desc
       '''
    if showsql:
        print(q)
